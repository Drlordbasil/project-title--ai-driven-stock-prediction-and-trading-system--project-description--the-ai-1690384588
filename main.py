...
def preprocess_data(stock_data, target_col):
    # Feature engineering
    stock_data['SMA'] = stock_data['Close'].rolling(window=20).mean()     # Simple Moving Average
    stock_data['RSI'] = calculate_rsi(stock_data['Close'])                # Relative Strength Index (RSI)
    stock_data['MACD'] = calculate_macd(stock_data['Close'])              # Moving Average Convergence Divergence (MACD)
...
def calculate_rsi(close_prices, window=14):
    ...
    up[up.index[window-1]] = np.mean(up[:window])  # first value is average of gains
    up = up.drop(up.index[:(window-1)])
    down[down.index[window-1]] = np.mean(down[:window])  # first value is average of losses
...
def train_lstm(X_train, y_train, epochs, batch_size):
    ...
    model.fit(X_train, y_train, epochs=epochs, batch_size=batch_size, verbose=1, callbacks=[EarlyStopping(patience=3)])
    return model
...
def main():
    ...
    random_forest_signals = generate_trading_signals(random_forest_model, X_test)
    lstm_signals = generate_trading_signals(lstm_model, X_test)
    
    # Performing backtesting and evaluating trading strategies
    random_forest_returns = perform_backtesting(y_test, random_forest_signals, initial_investment=10000)
    lstm_returns = perform_backtesting(y_test, lstm_signals, initial_investment=10000)
